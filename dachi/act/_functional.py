...
# #             results[i] = TaskStatus.from_bool(task)
# #         else:
# #             results[i] = task(reset=reset)

# #     for idx, task in enumerate(tasks):
# #         t = threading.Thread(target=run_task, args=(idx, task))
# #         threads.append(t)
# #         t.start()
...
# #     if success_on < 0:
# #         success_on = len(statuses) + 1 + success_on

# #     if fails_on < 0:
# #         fails_on = len(statuses) + 1 + fails_on

# #     if (fails_on + success_on) > (len(statuses) + 1):
# #         raise ValueError(
...
# #     if not preempt and (succeeded + failed) < len(statuses):
...
#         elif isinstance(task, bool):
#             statuses.append(TaskStatus.from_bool(task))
#         else:
#             statuses.append(task(reset=reset))
...
# # TODO: Add a non async version of parallel
# #       Add a threaded version of parallel
# #       
...
# def waitcondf(task: typing.Callable, *args, **kwargs) -> CALL_TASK:
#     """Functional form of action
