...
class Ref(object):
    ...
    def get(self, store: DataStore, default) -> typing.Any:
        """Get A value from the store

        Args:
            store (DataStore): The store to retrieve from
            default: The value to return if not in store

        Returns:
            typing.Any: The value retrieved
        """
        return store.get(self._, default)
    
    def get_or_set(self, store: DataStore, value):
        ...
        """
        store.get_or_set(self._name, value)
    
    def set(self, store: DataStore, value):
        """Set the value indexed by the ref

        Args:
            store (DataStore): The store to set to
...
def refs(names: typing.List[str]) -> typing.Tuple[Ref]:
    """Create several refs
