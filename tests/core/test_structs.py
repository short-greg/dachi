...
class TestModuleList:
    def test_itemlist_init_happy(self):
        lst = ModuleList(data=[make_leaf(), make_leaf()])
        assert len(lst) == 2
    ...
    def test_itemlist_init_type_error(self):
        with pytest.raises(TypeError):
            ModuleList(data=[42])             # not BaseProcess
    ...
    def test_itemlist_spec_roundtrip(self):
        lst = ModuleList(data=[make_leaf(3.3)])
        spec = lst.spec()
    ...
    def test_itemlist_state_dict_roundtrip(self):
        lst1 = ModuleList(data=[make_leaf(5), make_leaf(6)])
        sd = lst1.state_dict()
        lst2 = ModuleList(data=[make_leaf(0), make_leaf(0)])
    ...
    def test_itemlist_parameters_dedup(self):
        ...
        leaf2 = Leaf3(w=7, s=1)
        leaf2.w = leaf1.w
        lst = ModuleList(data=[leaf1, leaf2])
    ...
    def test_post_init_type_error(self):
        ...
        with pytest.raises(TypeError):
            ModuleList(data=["not a module"])


    def test_len_and_iter_and_getitem(self):
        leaf1 = Leaf4(v=1,f=1)
        leaf2 = Leaf4(v=2,f=2)
        ml = ModuleList(data=[leaf1, leaf2])
        assert len(ml) == 2
